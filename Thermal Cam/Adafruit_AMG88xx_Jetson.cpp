...
//#define I2C_DEBUG 
/*
Initial code taken from Adafruit AMG8833 arduino library, adapted by Daniel Staples for MECH5030M Team Project for project 415: Firefighting UAV drone, Feb 2019
 */ 
...
void Adafruit_AMG88xx::read(uint8_t reg, uint8_t *buf, uint8_t num)
{
	...
	while(pos < num){
		...
		i2c_smbus_read_byte(kI2CFileDescriptor, (uint8_t)reg + pos);
		

#ifdef I2C_DEBUG
		printf("[$"); printf(reg + pos, HEX); printf("] -> ");
#endif
		for(int i=0; i<read_now; i++){
			buf[pos] = i2c_smbus_read_byte();
#ifdef I2C_DEBUG
			printf("0x"); printf(buf[pos], HEX); printf(", ");
#endif
			pos++;
		}
#ifdef I2C_DEBUG
...
{
#ifdef I2C_DEBUG
		printf("[$"); printf(reg, HEX); printf("] <- ");
#endif
	i2c_smbus_write_byte(kI2CFileDescriptor, (uint8_t)reg);
	for (int i=0; i<num; i++) {
	  i2c_smbus_write_byte(kI2CFileDescriptor, buf[i]);
#ifdef I2C_DEBUG
	  printf("0x"); printf(buf[i], HEX); printf(", ");
#endif
	}
#ifdef I2C_DEBUG
	printf(" /n");
